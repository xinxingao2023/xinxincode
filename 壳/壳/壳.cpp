...
VOID writeFile(LPVOID NewBuffer, DWORD Size) {
	FILE* file;
	fopen_s(&file, "C://Users//GAOXINGXING//Desktop//5.0.exe", "wb");
	if (!file)
	{
		printf(" 无法打开 EXE 文件! ");
		exit(1);
...
DWORD NewRelaction(LPVOID pFileBuffer, HMODULE StubDll, LPVOID NewSectionFVA, DWORD NewSectionRVA)
 { 
	...
	//stub重定位表的VA
	PIMAGE_DATA_DIRECTORY	pDataDirectory = GetOptHeader(StubDll)->DataDirectory;
	PIMAGE_BASE_RELOCATION pRelocationDirectory = (PIMAGE_BASE_RELOCATION)(pDataDirectory[5].VirtualAddress + (DWORD)KImageBase);
	//拷贝stub的重定位表
...
VOID PrintRelocation(LPVOID pFileBuffer) {
	PIMAGE_DOS_HEADER pDosHeader = NULL;
	PIMAGE_NT_HEADERS pNTHeader = NULL;
	PIMAGE_FILE_HEADER pPEHeader = NULL;
	PIMAGE_OPTIONAL_HEADER32 pOptionHeader = NULL;
	...
	DWORD dwelocationRva = pDataDirectory[5].VirtualAddress;
	DWORD dwelocationFoa = RVAToFOA((LPVOID)dwelocationRva, pFileBuffer);
	PIMAGE_BASE_RELOCATION	pRelocation = (PIMAGE_BASE_RELOCATION)((DWORD)pFileBuffer + dwelocationFoa);
	DWORD  RelocationAddress;//重定位的地址
	DWORD  AddressAttribute;  //重定位地址的属性
	for (int a = 1; pRelocation->SizeOfBlock != 0 && pRelocation->VirtualAddress != 0; pRelocation = (PIMAGE_BASE_RELOCATION)((DWORD)pRelocation + pRelocation->SizeOfBlock)) {
		printf("++++++++++++++++++++++++++++++++++++++++++++++++++\n");
		printf("第%d块VirtualAddress:%x\n", a, pRelocation->VirtualAddress);
		printf("第%d块大小:%x\n", a, pRelocation->SizeOfBlock);
		//要修改地方的偏移
		PWORD ReDate = (PWORD)((DWORD)pRelocation + 8);
		for (int i = 0; i < (pRelocation->SizeOfBlock - 8) / 2; i++)
		{
			RelocationAddress = ((*ReDate) & 0xFFF) + pRelocation->VirtualAddress;
			//前四位是属性
			AddressAttribute = (*ReDate) >> 12;
			printf("属性：%d，RVA地址：%x\n", AddressAttribute, RelocationAddress);
...
int main()
{
	...
	for (int a = 0; a < pPEHeader->NumberOfSections; a++) {
		...
		if ( !strcmp(name, ".rsrc")|| !strcmp(name, "hhhh")) {
			Flag = FALSE;
		}
		if (Flag) {
