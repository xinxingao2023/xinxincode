...
{	
	...
	for (int a = 0; a < pPeh->NumberOfSections; a++) {
		...
		char name3[9] = "hhhh";
		BOOL Flag = TRUE;
		//(StrCmpText((char*)name, name1)) ||
		if ( (StrCmpText((char*)name, name2)) || (StrCmpText((char*)name, name3))) {
			Flag = FALSE;
		}
		if (Flag) {
...
PIMAGE_BASE_RELOCATION pRelocateTable = (PIMAGE_BASE_RELOCATION)(g_Sc.RelocationRVA+(DWORD)g_hModule);
	DWORD dwRelocOffset = (DWORD)g_hModule-GetOptHeader(g_hModule)->ImageBase;

	while (pRelocateTable->VirtualAddress != NULL)
		{      //定位到数据处+0x8
			PWORD parrOffset = (PWORD)((DWORD)pRelocateTable + sizeof(IMAGE_BASE_RELOCATION));
			//重定位数据个数，每一个数据是word类型
			DWORD dwRvaCount = (pRelocateTable->SizeOfBlock - sizeof(IMAGE_BASE_RELOCATION)) / 2;
			DWORD Old = 0;
			MyVirtualProtect((LPVOID)( + pRelocateTable->VirtualAddress), 0x10000, PAGE_READWRITE, &Old);
			for (DWORD i = 0; i < dwRvaCount; i++)
			{
				DWORD dwRva = parrOffset[i];
				//最高位nType != 3,不作处理
				...
				dwRva &= 0x0fff;
				//实际地址
				dwRva = (DWORD)((DWORD)g_hModule + pRelocateTable->VirtualAddress + dwRva);
...
void tlstable() {
	...
	if (g_Sc.TLStableRVA)
	{
		...
		auto CallBackTable = (PIMAGE_TLS_CALLBACK*)(g_Sc.CallBack);
		while (*CallBackTable)
		{
			MyMessageBoxW(0, 0, 0, 0);
